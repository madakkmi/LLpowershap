...
class ShapExplainer(ABC):
    ...
    def _fit_get_shap(self, X_train, Y_train, X_val, Y_val, random_seed, method, **kwargs) -> np.array:
    ...
    def explain(
        ...
        **kwargs,
        ...
        method: str
            Whether to use llpowershap or powershap
        **kwargs: dict
        ...
        # -----------------------------------------------------------
        random_col2_name = "random_normal_feature"     
        random_col3_name = "random_expo_feature"
        random_col4_name = "random_cauchy_feature"   
        random_col5_name = "random_logistic_feature"
        # -----------------------------------------------------------
        ...
        for i in iterations:
            ...
            if method == 'llpowershap':
                ...
                npRandomState2 = RandomState(random_seed)
                X[random_col2_name] = npRandomState2.normal(size=size)
                X[random_col3_name] = npRandomState2.standard_exponential(size=size)    
                X[random_col4_name] = npRandomState2.standard_cauchy(size=size)    
                X[random_col5_name] = npRandomState2.logistic(size=size)                          
...
class XGBoostExplainer(ShapExplainer):
    ...
    def _fit_get_shap(self, X_train, Y_train, X_val, Y_val, random_seed, method, **kwargs) -> np.array:
        ...
        PowerShap_model.fit(X_train, Y_train, eval_set=[(X_val, Y_val)], verbose=False)
        if method == 'powershap':
            C_explainer = shap.TreeExplainer(PowerShap_model)
            return C_explainer.shap_values(X_val, Y_val)
        else:
        
            C_explainer = shap.explainers.Tree(PowerShap_model, 
                                shap.sample(X_train, nsamples=1024,
                                random_state=random_seed),
                                feature_perturbation='interventional', 
                                model_output='log_loss')

            return C_explainer.shap_values(X_test, Y_test) * -1.0                    
