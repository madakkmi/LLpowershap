'''
Original authors: Jarne Verhaeghe, Jeroen Van Der Donckt
...
class PowerShap(SelectorMixin, BaseEstimator):
    ...
    def __init__(
        ...
        **fit_kwargs,
        ...
        power_alpha: float, optional
            ...
            0.01.
        method: str, optional
            The default method of feature selection, either 'powershap' or 'llpowershap'
            Default is 'llpowershap' for classification. 'llpowershap' is not available
            for regression. 

        val_size: float, optional
            The fractional size of the validation set. Should be a float between ]0,1[.
            By default 0.2. For powershap, default is 0.2 and for llpowershap, default
            is 0.3
    ...
    #@staticmethod
    def _check_llpowershap_regression(self, y: np.ndarray):

        assert isinstance(y, np.ndarray)
        dtype = y.dtype        

        if np.issubdtype(dtype, np.number) and not np.issubdtype(dtype, np.integer) and self.method == 'llpowershap':
            raise ValueError("LLpowershap is not available for regression")

    @staticmethod
    def _check_llpowershap_model(model, method):
        if model is not None:
            if not isinstance(model, XGBClassifier) and method == 'llpowershap':
                raise ValueError("LLpowershap is only available with XGBClassifier")

    @staticmethod
    def _get_default_model(y: np.ndarray, method):
        ...
        if np.issubdtype(dtype, np.number) and not np.issubdtype(dtype, np.integer):
            ...
            if method == 'powershap':
                ...
                return XGBClassifier(
                    n_estimators=250, early_stopping_rounds=25, verbosity=0
                )                
    ...
    def _automatic_fit(
        ...
        if not any(processed_shaps_df.p_value < self.power_alpha):
            ...
            self._print(f"No features selected after {loop_its} automatic iterations!")
        ...
        while (
            ...
            if max_iterations - max_iterations_old > self.limit_automatic:
                self._print(
                    f"Automatic mode: {self.method} requires {max_iterations} ",
                    "iterations; The extra required iterations exceed the limit_automatic ",
                    f"threshold. {self.method} will add ",
                    f"{self.limit_incremental_iterations} {self.method} iterations and ",
                    "re-evaluate.",
                )
    ...
    def fit(self, X, y, stratify=None, groups=None, **kwargs):
        ...
        # check if LLpowershap is specified for regression
        # results in an error if LLpowershap is specified for regression
        self._check_llpowershap_regression(np.asarray(y))

        # check LLpowershap is specified for non XGBoostClassifier model
        PowerShap._check_llpowershap_model(self.model, self.method)


        if self.model is None:
            # If no model is passed to the constructor -> select the default catboost
            # model
            self.model = PowerShap._get_default_model(np.asarray(y), method=self.method)
        ...
        self._print(f"Starting {self.method}")
        ...
        if self.automatic:
            ...
            self._print(
                f"Automatic mode enabled: Finding the minimal required {self.method}",
        ...
        processed_shaps_df = powerSHAP_statistical_analysis(
            shaps_df, self.power_alpha, self.power_req_iterations, include_all=self.include_all, method=self.method
        )
        ...
        if self.automatic:
            ...
            if self.force_convergence:
                ...
                while (len(converge_df[converge_df.p_value < self.power_alpha]) > 0) & (
                    ...
                    self._print(f"Rerunning {self.method} for convergence. ")
